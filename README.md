...
保存为 `~/.mimo/mimo.config.json` 后重新运行 `mimo` 即可生效。
...
export MIMO_BASE_URL="https://token-plan-sgp.xiaomimimo.com/anthropic"
```
...
虽然可以通过 `ANTHROPIC_BASE_URL` 和自定义请求头让 Claude Code 连接 MiMo API，但这种方式有根本性问题：
...
MiMo Code 启动时会按优先级读取：

1. `MIMO_CONFIG_FILE` 指定的 JSON 文件
2. `MIMO_CONFIG_DIR` 指定目录中的 `mimo.config.local.json` / `mimo.config.json`
3. `~/.mimo/mimo.config.local.json` / `~/.mimo/mimo.config.json`
4. MiMo Code 安装目录中的 `mimo.config.local.json` / `mimo.config.json`

专属环境变量会覆盖配置文件里的同名值，适合临时覆盖。出于安全考虑，MiMo Code 不会从任意项目工作目录读取 `mimo.config.json`，也不会内置默认 API Key。

| 变量 | 说明 | 默认值 |
|---|---|---|
| `MIMO_CONFIG_FILE` | 自定义配置文件路径 | 未设置 |
| `MIMO_CONFIG_DIR` | 自定义配置目录 | `~/.mimo` |
| `MIMO_API_KEY` | MiMo Token Plan API Key | 读取 MiMo 配置文件 |
| `MIMO_BASE_URL` | 自定义 API 端点 | `https://token-plan-sgp.xiaomimimo.com/anthropic` |
| `MIMO_HAIKU_MODEL` | 快速模型 ID | `mimo-v2.5` |
| `MIMO_SONNET_MODEL` | 默认模型 ID | `mimo-v2.5` |
| `MIMO_OPUS_MODEL` | 高能力模型 ID | `mimo-v2.5-pro` |
| `MIMO_SMALL_FAST_MODEL` | 小快模型 ID | `mimo-v2.5` |
...
```bash
bun run health   # 非联网本地健康检查
bun run smoke    # typecheck + build + bundle + 入口验证
```
