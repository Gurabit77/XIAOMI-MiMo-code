...
export function registerScheduleRemoteAgentsSkill(): void {
  registerBundledSkill({
    ...
    whenToUse:
      'When the user wants to schedule a recurring remote agent, set up automated tasks, create a cron job for MiMo Code, or manage their scheduled agents/triggers.',
    userInvocable: true,
    isEnabled: () =>
      ...
      if (repo === null) {
        ...
        if (!hasAccess) {
          ...
          const msg = webSetupEnabled
            ? `GitHub not connected for ${repo.owner}/${repo.name} \u2014 run /web-setup to sync your GitHub credentials, or install the MiMo GitHub App at https://claude.ai/code/onboarding?magic=github-app-setup.`
            : `MiMo GitHub App not installed on ${repo.owner}/${repo.name} \u2014 install at https://claude.ai/code/onboarding?magic=github-app-setup if your trigger needs this repo.`
          setupNotes.push(msg)
        }
      }
