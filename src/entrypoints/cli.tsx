#!/usr/bin/env bun
import './mimo-bootstrap.js' // MUST be first — sets env vars before any module caches them
import { feature } from 'bun:bundle'
import { isEnvTruthy } from '../utils/envUtils.js'

// MiMo model display names for /model picker
...
if (!process.env.ANTHROPIC_DEFAULT_HAIKU_MODEL_DESCRIPTION) {
  process.env.ANTHROPIC_DEFAULT_HAIKU_MODEL_DESCRIPTION = 'MiMo V2.5 · Fast & efficient coding'
}
...
if (!process.env.ANTHROPIC_DEFAULT_SONNET_MODEL_DESCRIPTION) {
  process.env.ANTHROPIC_DEFAULT_SONNET_MODEL_DESCRIPTION = 'MiMo V2.5 · Best for everyday coding tasks'
}
...
if (!process.env.ANTHROPIC_DEFAULT_OPUS_MODEL_DESCRIPTION) {
  process.env.ANTHROPIC_DEFAULT_OPUS_MODEL_DESCRIPTION = 'MiMo V2.5 Pro · Most capable, 1M context'
}
// Additional MiMo models for /model picker
if (!process.env.ANTHROPIC_CUSTOM_MODEL_OPTION) {
  process.env.ANTHROPIC_CUSTOM_MODEL_OPTION = 'MiMo-V2-Omni'
}
if (!process.env.ANTHROPIC_CUSTOM_MODEL_OPTION_NAME) {
  process.env.ANTHROPIC_CUSTOM_MODEL_OPTION_NAME = 'MiMo V2 Omni'
}
if (!process.env.ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION) {
  process.env.ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION = 'MiMo V2 Omni · Multimodal: image, video, audio'
}
if (!process.env.API_TIMEOUT_MS) {
  process.env.API_TIMEOUT_MS = '3000000'
}
// ===== End MiMo Code defaults =====
...
async function main(): Promise<void> {
  ...
  if (
    ...
    console.log(`${MACRO.VERSION} (MiMo Code)`)
    return
  }
