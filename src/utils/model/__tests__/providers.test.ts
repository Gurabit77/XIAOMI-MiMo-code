...
describe('getAPIProvider', () => {
  ...
  test('returns "mimo" for MiMo runtime', () => {
    process.env.MIMO_CODE_RUNTIME = '1'
    expect(getAPIProvider()).toBe('mimo')
  })

  test('MiMo runtime takes precedence over modelType and provider env vars', () => {
    mockedModelType = 'gemini'
    process.env.MIMO_CODE_RUNTIME = '1'
    process.env.CLAUDE_CODE_USE_BEDROCK = '1'
    expect(getAPIProvider()).toBe('mimo')
...
describe('isFirstPartyAnthropicBaseUrl', () => {
  ...
  const originalMiMoRuntime = process.env.MIMO_CODE_RUNTIME
  ...
  afterEach(() => {
    ...
    if (originalMiMoRuntime !== undefined) {
      process.env.MIMO_CODE_RUNTIME = originalMiMoRuntime
    } else {
      delete process.env.MIMO_CODE_RUNTIME
    }
  })
  ...
  test('returns false in MiMo runtime', () => {
    process.env.MIMO_CODE_RUNTIME = '1'
